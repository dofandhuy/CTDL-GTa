...
//===========================================================================
  void depth2(boolean[] visited, int k, RandomAccessFile f, int[] count) throws Exception {
    if (count[0] >= 5) return; // Chỉ in tối đa 5 đỉnh
    fvisit(k, f);
    visited[k] = true;
    count[0]++;
...
void depth2(int k, RandomAccessFile f) throws Exception {
    boolean[] visited = new boolean[20];
    int[] count = {0}; // Mảng 1 phần tử để lưu số đỉnh đã in
    for (int i = 0; i < n; i++) visited[i] = false;

    depth2(visited, k, f, count);
    
    for (int i = 0; i < n && count[0] < 5; i++) {
        if (!visited[i]) {
            depth2(visited, i, f, count);
        }
    }
